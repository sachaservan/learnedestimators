...
from utils import get_data, get_stat, git_log, get_data_str_with_ports_list
from greedy import count_min, cutoff_countmin, order_y_wkey
from aol_utils import get_data_aol_query
...
if __name__ == '__main__':
    ...
    argparser.add_argument("--n_buckets_list", type=int, nargs='*', help="number of buckets", default=[])
    ...
    argparser.add_argument("--use_ports", action='store_true', default=False)
    args = argparser.parse_args()

    assert not (args.space_list and args.n_buckets_list), 'n_buckets are not gonna be used'
    assert not (not args.space_list and not args.n_buckets_list), 'use either --n_buckets_list or --space_list'
    ...
    if args.space_list:
        ...
        for n_hash in args.n_hashes_list:
            for n_buckets in args.n_buckets_list:
                nh_all.append(n_hash)
                nb_all.append(n_buckets)
        rshape = (len(args.n_hashes_list), len(args.n_buckets_list))
