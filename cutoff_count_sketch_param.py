...
from greedy import cutoff_countsketch, cutoff_lookup, cutoff_countsketch_wscore, order_y_wkey_list
from aol_utils import get_data_aol_query_list


def run_ccs(y, b_cutoff, n_hashes, n_buckets, name):
    start_t = time.time()
...
def run_ccs_wscore(y, scores, score_cutoff, n_cm_buckets, n_hashes, name):
    start_t = time.time()
...
def run_ccm_lookup(x, y, n_hashes, n_cm_buckets, d_lookup, s_cutoff, name):
    start_t = time.time()
...
def get_good_cut(b_cut, y_train):
    '''make b_cut at the boundary of two item numbers'''
    y_train_sorted = np.sort(y_train)[::-1]
    if b_cut < len(y_train_sorted):
        s_cut = y_train_sorted[b_cut]
    else:
        s_cut = y_train_sorted[-1]
    b_cut_new = np.argmax(y_train_sorted == s_cut) # first item that # items == s_cut
...
if __name__ == '__main__':
    ...
    assert not (args.word_data and args.aol_data),          "use either --word or --aol"
    ...
    if args.perfect_order:
        name = 'cutoff_csketch_param_perfect'
    elif args.lookup_data:
        name = 'lookup_table_csketch'
    else:
        name = 'cutoff_csketch_param'
        ...
        for bcut in b_cutoffs:
            for n_hash in args.n_hashes_list:
                ...
                if args.perfect_order:
                    ...
                    # version 1
                    #bcut, scut = get_great_cut(bcut, y_valid, np.floor(max_bcut))
                elif args.lookup_data:
    ...
    if args.perfect_order:
        ...
        results = pool.starmap(run_ccs, zip(repeat(y_sorted), bcut_all, nh_all, nb_all, repeat(name)))
    elif args.lookup_data:
        results = pool.starmap(run_ccm_lookup, zip(repeat(x_valid), repeat(y_valid), nh_all, n_cm_all, repeat(lookup_dict), scut_all, repeat(name)))
    else:
        results = pool.starmap(run_ccs, zip(repeat(y_valid_ordered), bcut_all, nh_all, nb_all, repeat(name)))
    pool.close()
    pool.join()
    ...
    if args.perfect_order:
        ...
        results = pool.starmap(run_ccs, zip(repeat(y_sorted), best_bcuts, best_n_hashes, best_n_buckets, repeat(name)))
        ...
        results = pool.starmap(run_ccs_wscore,
            zip(repeat(y_test_ordered), repeat(test_scores), best_scuts, best_n_buckets - best_bcuts, best_n_hashes, repeat(name)))
    pool.close()
    pool.join()
