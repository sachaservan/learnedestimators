...
def loss_relative(loss, y_true, y_est):
    return loss + np.abs(y_true - y_est) / y_true
...
loss_functions = [loss_l1, loss_l2, loss_weighted, loss_relative]
loss_labels = ["Average Loss per item (L1)", "Average Loss (L2)", "Average Loss (Weighted)", "Average Loss (Relative)"]
###########################
...
if __name__ == '__main__':
    ...
    save_file_names = ["experiments/loss_l1_ip.pdf", "experiments/loss_l2_ip.pdf", "experiments/loss_weighted_ip.pdf", "experiments/loss_relative_ip.pdf"]
    if args.aol:
        save_file_names = ["experiments/loss_l1_aol.pdf", "experiments/loss_l2_aol.pdf", "experiments/loss_weighted_aol.pdf", "experiments/loss_relative_aol.pdf"]
    elif args.synth:
        save_file_names = ["experiments/loss_l1_synth.pdf", "experiments/loss_l2_synth.pdf", "experiments/loss_weighted_synth.pdf", "experiments/loss_relative_synth.pdf"]
       
    results_regular = np.load(args.results,  allow_pickle=True)
    ...
    algo_predictions = np.array(results_regular['algo_predictions'])
    count_sketch_predictions = np.array(results_regular['count_sketch_predictions'])
    cutoff_count_sketch_predictions = np.array(results_regular['cutoff_count_sketch_predictions'])
    ...
    for i in range(len(loss_functions)):
        ...
        for j in range(len(space)):
            loss_for_space_constant = 0
            loss_for_space_algo = 0
            loss_for_space_sketch = 0
            loss_for_space_cutoff_count_sketch = 0

            constant = np.median(true_counts)
            for k in range(len(true_counts)):
                loss_for_space_constant = loss_functions[i](loss_for_space_constant, true_counts[k], constant)
            ...
            loss_constant.append(loss_for_space_constant)
        ...
        loss_constant = np.array(loss_constant) / total
        ...
        ax.plot(space_percent, loss_constant, label="Constant", color=colors[4], zorder=6)
