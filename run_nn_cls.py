...
from utils import get_stat, git_log, AverageMeter, get_data_args, get_data_list_args, keep_latest_files
...
def construct_graph(args):
    with tf.variable_scope("nhh"):
        ...
        if args.rnn_hiddens:
        ...
        elif args.port_hiddens:
            # separate encodes for src and dst ips
            n_feat_ip = 32
            enc1_hidden_len = [n_feat_ip] + args.sep_hiddens
            enc2_hidden_len = [n_feat_ip] + args.sep_hiddens

            feat_src_ip = feat[:,:n_feat_ip]
            feat_dst_ip = feat[:,n_feat_ip:n_feat_ip*2]
            src_ip_out, _, _ = fc_layers(feat_src_ip, enc1_hidden_len, np.ones(len(enc1_hidden_len)), name='fc_encoder_src_ip', activation=args.activation, summary_layers=[])
            dst_ip_out, _, _ = fc_layers(feat_dst_ip, enc2_hidden_len, np.ones(len(enc2_hidden_len)), name='fc_encoder_dst_ip', activation=args.activation, summary_layers=[])
            ...
            # concat all
            hidden_len = [args.sep_hiddens[-1]*2 + args.port_hiddens[-1]*2 + 1] + args.hiddens + [1]
            output = tf.concat([src_ip_out, dst_ip_out, src_port_out, dst_port_out, proto_out], axis=1)
            output, weights, bias = fc_layers(output, hidden_len, keep_probs, name='fc_encoder', activation=args.activation, summary_layers=[])

        elif args.sep_hiddens:
            # separate encodes for src and dst ips
            n_feat_sep = int(args.n_feat / 2)
            assert args.n_feat % 2 == 0
            enc1_hidden_len = [n_feat_sep] + args.sep_hiddens
            enc2_hidden_len = [n_feat_sep] + args.sep_hiddens
            hidden_len = [args.sep_hiddens[-1]*2] + args.hiddens + [1]

            feat1 = feat[:,:n_feat_sep]
            feat2 = feat[:,n_feat_sep:]
            f1_out, _, _ = fc_layers(feat1, enc1_hidden_len, np.ones(len(enc1_hidden_len)), name='fc_encoder_1', activation=args.activation, summary_layers=[])
            f2_out, _, _ = fc_layers(feat2, enc2_hidden_len, np.ones(len(enc2_hidden_len)), name='fc_encoder_2', activation=args.activation, summary_layers=[])
            output = tf.concat([f1_out, f2_out], axis=1)
            output, weights, bias = fc_layers(output, hidden_len, keep_probs, name='fc_encoder', activation=args.activation, summary_layers=[])
        else:
            # fully connected encoder
            hidden_len = [args.n_feat] + args.hiddens + [1]
            ...
            output, weights, bias = fc_layers(feat, hidden_len, keep_probs, name='fc_encoder', activation=args.activation, summary_layers=enc_sum)
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--hiddens', type=int, nargs='*', default=[], help="Examples: --hiddnes 144 288")
    ...
    argparser.add_argument('--bin_feat', action='store_true', default=True)
    ...
    argparser.add_argument('--sep_hiddens', type=int, nargs='*', default=[], help="Examples: --hiddnes 144 288")
    argparser.add_argument('--port_hiddens', type=int, nargs='*', default=[], help="Examples: --hiddnes 144 288")
    argparser.add_argument('--rnn_hiddens', type=int, nargs='*', default=[], help="Examples: --hiddnes 144 288")
    ...
    if not args.port_hiddens:
        feat_idx = np.arange(8)
    else:
        feat_idx = np.arange(11)

    if not args.bin_feat:
        args.n_feat = len(feat_idx)
    else:
        if args.port_hiddens:
            args.n_feat = 8*8+2*16+1    # ip-> 8*8, port-> 2*16, protocol->1
        else:
            args.n_feat = len(feat_idx) * 8
    start_t = time.time()

    train_x, train_y = get_data_args(args.train, feat_idx, args)
    print('train x shape:', train_x.shape, 'y max', np.max(train_y), 'y min', np.min(train_y))

    valid_x, valid_y = get_data_args(args.valid, feat_idx, args)
    print('valid x shape:', valid_x.shape, 'y max', np.max(valid_y), 'y min', np.min(valid_y))

    test_x, test_y = get_data_list_args(args.test, feat_idx, args)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess, open('log/'+str(args.save_name)+'_'+time_now+'_'+str(args.seed)+'.log','w') as fp:
        ...
        if not args.evaluate:
            ...
            test_output_all = []
            for i in range(len(test_x)):
                test_loss, test_output = evaluate(test_x[i], test_y[i], args, sess)
